...
open class BarChartRenderer: BarLineScatterCandleBubbleRenderer
{
    ...
    @objc open func drawDataSet(context: CGContext, dataSet: IBarChartDataSet, index: Int)
    {
        ...
        for j in stride(from: 0, to: buffer.rects.count, by: 1)
        {
            ...
            var isTopValueInStack = false
            if dataSet.isStacked,
               let stackValues = (dataSet.entryForIndex(j / dataSet.stackSize) as? BarChartDataEntry)?.yValues {
                let stackIndex = j % dataSet.stackSize
                if stackValues[stackIndex] != 0 {
                    if stackIndex == dataSet.stackSize - 1 {
                        isTopValueInStack = true
                    } else if stackValues[(stackIndex + 1)...].allSatisfy({ $0 == 0 }) {
                        isTopValueInStack = true
                    }
                }
            }

            let cornerRadius = barRect.size.width / 4.0
            if !dataSet.isStacked {
                let bezierPath = UIBezierPath(roundedRect: barRect, byRoundingCorners: dataSet.entryForIndex(j)?.y ?? 0 > 0 ? [.topLeft, .topRight] : [.bottomLeft, .bottomRight], cornerRadii: CGSize(width: cornerRadius, height: cornerRadius))
                bezierPath.fill()
            } else if dataSet.isStacked && isTopValueInStack {
    ...
    open override func drawHighlighted(context: CGContext, indices: [Highlight])
    {
        ...
        for high in indices
        {
            ...
            if let e = set.entryForXValue(high.x, closestToY: high.y) as? BarChartDataEntry
            {
                ...
                let stackSize = e.yValues?.count ?? 1

                let index = set.entryIndex(entry: e)
                let fullIndex = isStack ? (index * stackSize) + high.stackIndex : index
                let color = set.color(atIndex: fullIndex)
                ...
                var isTopValueInStack = false
                if set.isStacked,
                   let stackValues = (set.entryForIndex(fullIndex / set.stackSize) as? BarChartDataEntry)?.yValues {
                    if high.stackIndex >= 0 && stackValues[high.stackIndex] != 0 {
                        if high.stackIndex == set.stackSize - 1 {
                            isTopValueInStack = true
                        } else if stackValues[(high.stackIndex + 1)...].allSatisfy({ $0 == 0 }) {
                            isTopValueInStack = true
                        }
                    }
                } else {
                ...
                let cornerRadius = barRect.size.width / 4.0
                if !isStack {
                    let bezierPath = UIBezierPath(roundedRect: barRect, byRoundingCorners: e.y > 0 ? [.topLeft, .topRight] : [.bottomLeft, .bottomRight], cornerRadii: CGSize(width: cornerRadius, height: cornerRadius))
                    bezierPath.fill()
                } else if isStack && isTopValueInStack {
                    let bezierPath = UIBezierPath(roundedRect: barRect, byRoundingCorners: [.topLeft, .topRight], cornerRadii: CGSize(width: cornerRadius, height: cornerRadius))
                    bezierPath.fill()
                } else {
                    context.fill(barRect)
